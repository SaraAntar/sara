...
//The variables email_in_use, password_in_use, and who are declared to keep state information about who is logged in. 
//These variables will be used to store the email, password, and user information of the currently logged-in user.
var email_in_use = "";
var password_in_use = "";
var who = "";
//app.set it is function used to set the application setting properties. 
var app = express();
...
app.get('/checkIfApptExists', (req, res) => {
  ...
  let ndate = new Date(date).toLocaleDateString().substring(0, 10)//line converts the date string to a JavaScript Date object, formats it as a localized date string, and extracts only the first 10 characters. This is done to ensure that the date is in the desired format for further processing.
  let sql_date = `STR_TO_DATE('${ndate}', '%d/%m/%Y')`;
  //sql to turn string to sql time obj
  let sql_start = `CONVERT('${startTime}', TIME)`;
  //The sql_date and sql_start variables are created to construct the SQL queries for comparing dates and times in the database. 
  //They use the extracted query parameters and format them appropriately for the SQL syntax.
...
//The code  deals with handling a GET request to the /schedule endpoint and performing database operations
// based on the query parameters. It constructs SQL queries using the extracted query parameters 
//and executes them using the con.query() method. The results are then returned as a JSON response.
